function roundness= forclass8(img_route,armon,dist_max,im_name,folder,display)
...
while lenx>2
    ...
    if length(re)>10
        ...
        [xc,yc,R,~] = circfit(x(re(mag(re)>0.5*mean(mag))),y(re(mag(re)>0.5*mean(mag))));
        if R<=1*radii
